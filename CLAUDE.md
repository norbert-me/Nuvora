...
- **Themenstand rechnet gewichtet, nicht gemittelt.** `apps/api/app/themenprofil.py` beantwortet „wie sicher sitzt dieses Unterthema, und wird es besser?" aus Klassenarbeiten **und** CardVote-Quizzen: erreichte durch mögliche Punkte je Thema (eine 8-Punkte-Aufgabe wiegt achtmal so schwer wie eine 1-Punkt-Frage — sie prüft auch mehr). Unter `MINDEST_PUNKTE` (6) gibt es **keine** Zahl, sondern „zu wenig für eine Aussage". Der Trend vergleicht die erste Hälfte der Erhebungen mit der zweiten, nie die letzte mit der vorletzten — sonst wäre jede schwache Arbeit ein „Abstieg". Die Note ist bewusst nur Beiwerk (`scoring.note_aus_pct`, dieselbe Funktion wie im Notenbuch — die zweite Fassung in `noten.py` wurde dafür aufgelöst): ein Notenschlüssel gilt für eine ganze Arbeit, nicht für drei Aufgaben daraus. Regressionstest `apps/api/tests/test_themenprofil.py`.
