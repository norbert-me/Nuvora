...
import { cardStyle, chipStyle, pageTitle, pageApp, COLORS as C } from "../components/Icons.jsx";
...
export default function ThemaAnsicht() {
  ...
  const row = { display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderTop: "1px solid var(--border)", fontSize: 14 };
  const a = data.active || {};
  ...
  return (
    <div style={{ ...pageApp }}>
      ...
      <p style={{ fontSize: 14, color: "var(--text2)", marginBottom: 16 }}>{t("thema.hint")}</p>
        ...
        {(data.cardvote || []).map((q) => (
          <Zeile key={q.id} to={themaZiel.cardvote(q)}>
            ...
            {/* Ohne das sehen zwei gleich lautende Fragen wie ein Fehler der
                ...
                in keinem. */}
            <span style={{ fontSize: 12, color: q.set_id ? "var(--text3)" : C.warning, flexShrink: 0 }}>
              {q.set_id ? q.set_name : t("thema.noSet")}
            </span>
          </Zeile>
        ))}
        <div style={{ marginTop: 8 }}><Link to="/cardvote/questions" style={{ color: "var(--accent)", fontSize: 13, textDecoration: "none" }}>{t("thema.openCardvote")} ↗</Link></div>
      </Section>
      ...
      <Section show={a.karten} title={t("thema.karten")} count={(data.karten || []).length} empty={t("thema.empty")}>
        {(data.karten || []).map((d) => (
          <Zeile key={d.id} to={themaZiel.karten(d)}>
            ...
            {!d.released && <span style={{ fontSize: 12, color: "var(--text3)" }}>{t("thema.draft")}</span>}
          </Zeile>
        ))}
      </Section>
      ...
      <Section show={a.lernpfad} title={t("thema.lernpfad")} count={(data.lernpfad || []).length} empty={t("thema.empty")}>
        ...
        <div style={{ marginTop: 8 }}><Link to="/lernpfad" style={{ color: "var(--accent)", fontSize: 13, textDecoration: "none" }}>{t("thema.openLernpfad")} ↗</Link></div>
      </Section>
      ...
      {!a.cardvote && !a.karten && !a.lernpfad && !a.kalender && !a["code-detektiv"] && (
        <p style={{ fontSize: 14, color: "var(--text3)" }}>{t("thema.noModulesButMaterial")}</p>
      )}
    </div>
  );
}
