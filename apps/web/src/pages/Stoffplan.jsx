...
export default function Stoffplan() {
  ...
  const add = async () => {
    ...
    await fetch(API, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ kurs_id: kursId ?? null, class_id: kursId != null ? null : classId, title: title.trim(), kw: kw.trim(), hours: hours ? Number(hours) : null }) }).catch(() => {});
    setTitle(""); setKw(""); setHours(""); load();
  };
