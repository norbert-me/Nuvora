      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', padding: 24 }}>
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            ...
            <p style={{ color: 'var(--text2)' }}>{t('cd.session', 'Session')} {session.id}</p>
          </div>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
        <div style={{ textAlign: 'center', maxWidth: 450 }}>
          ...
          <div style={{ ...cardStyle, padding: 20, textAlign: 'left' }}>
            <h3 style={{ fontSize: 14, color: 'var(--text3)', marginBottom: 12 }}>{t('cd.spieler_n', 'Spieler ({{n}})', { n: session.players.length })}</h3>
            {session.players.length === 0 && (
              <p style={{ color: 'var(--text3)', fontSize: 13 }}>{t('cd.play.keine_spieler', 'Noch keine Spieler beigetreten')}</p>
            )}
            {session.players.map((p, i) => (
              <div key={i} style={{ padding: '8px 0', borderBottom: i < session.players.length - 1 ? '1px solid var(--border)' : 'none', fontSize: 15 }}>
                {p.name} {p.name === playerName && <span style={{ color: 'var(--accent)' }}>{t('cd.play.du', '(Du)')}</span>}
              </div>
            ))}
          </div>
          <p style={{ marginTop: 16, fontSize: 13, color: 'var(--text3)' }}>{t('cd.play.admin_startet', 'Der Admin startet das Rätsel gleich...')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
        <div style={{ textAlign: 'center', maxWidth: 400 }}>
          ...
          <p style={{ color: 'var(--text2)', marginBottom: 24 }}>{t('cd.play.kein_beitritt', 'Du kannst dieser Session nicht mehr beitreten.')}</p>
          <button className="btn btn-primary" onClick={() => navigate(base)}><IconBack size={14} /> {t('cd.zur_startseite', 'Zurück zur Startseite')}</button>
        </div>
      </div>
    );
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
        <div style={{ textAlign: 'center', maxWidth: 400 }}>
          ...
          <p style={{ color: 'var(--text2)', marginBottom: 8 }}>{currentPuzzle.title}</p>
          <p style={{ color: 'var(--text3)', fontSize: 13, marginBottom: 24 }}>{currentPuzzle.description}</p>
          <button
            ...
            {session.players.map((p, i) => {
              ...
              return (
                <div key={i} className="player-list-item">
                  <span>{p.name}</span>
                  <span style={{ fontSize: 13, color: pr ? (pr.solved ? COLORS.success : COLORS.danger) : 'var(--text3)' }}>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', padding: 24 }}>
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <p style={{ color: 'var(--text2)' }}>{currentPuzzle?.title}</p>
          </div>
          ...
          {!isAdmin && (
            <p style={{ textAlign: 'center', marginTop: 24, color: 'var(--text3)' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
      <div style={{ textAlign: 'center' }}>
        <p style={{ color: 'var(--text3)' }}>{t('cd.laden', 'Lade...')}</p>
      </div>
    </div>
  );
}
