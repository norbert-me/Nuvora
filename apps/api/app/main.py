...
def _ensure_columns(sync_conn):
    ...
    wanted = [
        ...
        ("card_decks", "mischen", "BOOLEAN DEFAULT FALSE NOT NULL"),
        ...
        ("grade_categories", "source_work_id", "INTEGER"),
        ...
        ("exam_dates", "notiz", "TEXT DEFAULT '' NOT NULL"),
        ...
        ("calendar_entries", "location", "VARCHAR(200) DEFAULT '' NOT NULL"),
        ("calendar_entries", "rrule", "VARCHAR(200) DEFAULT '' NOT NULL"),
        ("calendar_entries", "exdate", "JSON"),
...
async def startup():
    ...
    # stehen, solange die Module darauf rechnen.
    async with async_session() as db:
        try:
            n = await uebernahme_personen(db)
            if n:
                print(f"[STARTUP] Personen: {n} Person(en) aus dem Bestand angelegt.", flush=True)
        except Exception as e:
            await db.rollback()
            print(f"[STARTUP-WARN] Personen-Uebernahme uebersprungen: {type(e).__name__}: {e}", flush=True)
